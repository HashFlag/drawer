...
import random,json,datetime,smtplib,io
...
def register(request):
    ...
    if obj.is_valid():
        ...
        ret.status = True
        userinfo_dict={"nid":userinfo.nid,"username":userinfo.username,"email":userinfo.email,"pwd":userinfo.pwd}
...
def login(request):
    ...
    if log.is_valid():
        ...
        if code.lower() != form['checkCodes'].lower():
            ret.registerError['checkCode'] = [{"message":"验证码错误"}]
            return HttpResponse(json.dumps(ret.__dict__))
        else:
            ...
            if not coun:
                ...
                request.session["is_login"] = form['username']
                ret.status = True
    else:
